...
if(isset($_SESSION['id'])){
	...
	$result = $conn->query($sql);
	while($row = $result->fetch_assoc()){
		$sql = "INSERT INTO `order`(CustomerID, CourseID, DatePurchase, Quantity, TotalPrice, Status) 
		VALUES(".$row['CustomerID'].", '".$row['CourseID']
	...
	$sql = "SELECT customer.CustomerName, customer.CustomerIC, customer.CustomerGender, customer.CustomerAddress, customer.CustomerEmail, customer.CustomerPhone, course.courseTitle, course.Price, course.Image, `order`.`DatePurchase`, `order`.`Quantity`, `order`.`TotalPrice`
		FROM customer, Course, `order`
		WHERE `order`.`CustomerID` = customer.CustomerID AND `order`.`CourseID` = course.courseID AND `order`.`Status` = 'N' AND `order`.`CustomerID` = ".$cID."";
	$result = $conn->query($sql);
...
<?php
	...
	while($row = $result->fetch_assoc()){
    	...
    	echo $row['courseTitle']."<br>Rs.".$row['Price']."<br>";
